...
```bash
cd <项目目录>
ocr review --from <源分支> --to <目标分支> --format json --audience agent --rule .rule/java-essential.json
```

**参数说明**：
- `--from <源分支>`：基准分支，如 main、develop（默认 main）
- `--to <目标分支>`：目标分支，即当前要审查的分支（默认当前分支）

> 注意：本技能仅支持分支间对比审查。`--from` 和 `--to` 由调用方传入，支持任意分支名。
